...
static int streebog_init(struct shash_desc *desc)
{
	...
	for (i = 0; i < 8; i++) {
		if (digest_size == STREEBOG256_DIGEST_BYTES)
...
static inline void pad(struct streebog_state *ctx)
{
	if (ctx->bufsize > 63)
		return;
...
static inline void add512(const union uint512_u *x, const union uint512_u *y,
    union uint512_u *r)
{
#ifdef __LITTLE_ENDIAN
	unsigned int CF, OF;
...
static inline void stage2(struct streebog_state *ctx, const unsigned char *data)
{
	g(&(ctx->h), &(ctx->N), data);
...
static inline void stage3(struct streebog_state *ctx)
{
...
static int streebog_update(struct shash_desc *desc, const u8 *data, unsigned int len)
{
	struct streebog_state *ctx = shash_desc_ctx(desc);
	size_t chunksize;
...
static int streebog_final(struct shash_desc *desc, u8 *digest)
{
	...
	stage3(ctx);
	ctx->bufsize = 0;
	if (crypto_shash_digestsize(desc->tfm) == STREEBOG256_DIGEST_BYTES)
...
static int alg_test_streebog(void)
{
	...
	for (t = streebog_tv; t->algo; t++) {
		...
		if (IS_ERR(tfm)) {
			pr_err("failed to load transform %s\n", algo);
			err = PTR_ERR(tfm);
			return err;
		}
...
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("Streebog Hash Functions");

MODULE_ALIAS_CRYPTO("sb256");
MODULE_ALIAS_CRYPTO("sb256-generic");
MODULE_ALIAS_CRYPTO("sb512");
MODULE_ALIAS_CRYPTO("sb512-generic");
